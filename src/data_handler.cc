...
void data_handler::read_feature_vector(std::string path) {
    uint32_t header[4]; // |MAGIC|NUM IMAGES|ROWSIZE|COLZIE
    ...
    if (f)
    {
        ...
        printf("Successfully read and stored %lu feature vectoprs.\n", data_array->size());
    } else {
...
void data_handler::split_data() {
    ...
    int train_size = data_array->size() * TRAIN_SET_PRECENT;
    ...
    {
        int rand_index = rand() % data_array->size(); // o & data_array_.size() -1
